var ApplicationEmbed =
  ...
  (function (w, u) {
    ...
    return {
      init: function () {
        var socialwall =
          document.getElementsByClassName("taggbox-socialwall").length;

        var container =
          socialwall && socialwall > 0
            ? document.getElementsByClassName("taggbox-socialwall")
            : document.getElementsByClassName("taggbox");

        for (var i = 0; i < container.length; i++) {
          if (
            container[i].getAttribute("data-is-load") == undefined ||
            container[i].getAttribute("data-is-load") == "0" ||
            container[i].getAttribute("data-is-load") == null
          ) {
            const url = AppPath;
